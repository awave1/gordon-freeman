...
data SECDInstruction
  ...
  | CLOS Prog Env -- closure operation, with closu
...
-- code, env, stack
type SECDMachine = (Prog, Env, Stack)
...
compile expr = case expr of
  ...
  (DFix term  ) -> [FIX (compile term ++ [RET])]
  _             -> undefined
 where
...
step machine = case machine of
  ...
  (APP : code, env, FIXCLOS fixCode fixEnv : var : stack) ->
    (fixCode, var : FIXCLOS fixCode fixEnv : fixEnv, CLOS code env : stack)

  (APP : code, env, FIXCLOS fixCode fixEnv : stack) ->
    (fixCode, FIXCLOS fixCode fixEnv : fixEnv, CLOS code env : stack)
  ...
  (CALL c        : code, env, stack      ) -> (c, env, CLOS code env : stack)

  (FIX  c        : code, env, stack      ) -> (code, env, FIXCLOS c env : stack)

  (FIXC c : code, env, stack) -> (code, env, FIXCCLOS c env : stack)
